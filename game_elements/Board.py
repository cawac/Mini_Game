...
class Board(Field):
    def __init__(self, path_to_background, position_of_locked_blocks, requirements, n, m, start_x=0, start_y=0):
        super().__init__(path_to_background, n, m, start_x, start_y)
        for i in position_of_locked_blocks:
            self.map[i[0]][i[1]] = LockedBlock(i[1] * self.block_rect.width + 5 * (i[1] + 1) + start_x,
                                               i[0] * self.block_rect.height + 5 * (i[0] + 1) + start_y)
        ...
        while needed_blocks:
            ...
            self.map[posy][posx] = MovableBlock(posx * self.block_rect.width + 5 * (posx + 1) + start_x,
                                                posy * self.block_rect.height + 5 * (posy + 1) + start_y, type_of_Block)
